    ...
    while (true)
    {
        ...
        if (!printed)
        {
            ...
            if (tutors.getSize() == 0)
            {
                ...
                cout << "Khong tim thay gia su phu hop!";
...
void FilterResult(Student *student, string TutorName, string SubjectName, string Address, string LowerCost, string UpperCost)
{
    ...
    if (!SubjectName.empty())
    {
        ...
        for (int i = 0; i < Result.getSize(); ++i)
        {
            ...
            {
                bool valid = subjectList[j]->GetSubject()->GetName().find(SubjectName) != string::npos;
                if(!LowerCost.empty())
                {
                    valid &= subjectList[j]->GetSubject()->GetCost() >= stoi(LowerCost);
                }
                if(!UpperCost.empty())
                {
                    valid &= subjectList[j]->GetSubject()->GetCost() <= stoi(UpperCost);
                }
                if (valid)
                {
