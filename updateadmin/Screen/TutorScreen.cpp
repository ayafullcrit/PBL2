...
void Screen_Tutor_4(Tutor *tutor)
{
    ...
    while (1)
    {
        ...
        for (int i = startIndex; i < endIndex; i++)
        {
            ...
            // Cột tên học viên - xử lý nhiều dòng
            string studentNames = stdnames[i];
            size_t pos = 0;
            int lineOffset = 0;
            while (pos < studentNames.length())
            {
                size_t endPos = studentNames.find('\n', pos);
                if (endPos == string::npos)
                    endPos = studentNames.length();
                
                string line = studentNames.substr(pos, endPos - pos);
                gotoXY(contentX + 35, currentY + lineOffset);
                cout << line;
                
                pos = endPos + 1;
                lineOffset++;
            }
            currentY += lineOffset > 0 ? lineOffset : 1;
        }

        // Thông tin phân trang
        string pageInfo1 = "Hien thi " + to_string(startIndex + 1) + "-" + to_string(endIndex);
        string pageInfo2 = "tren tong " + to_string(totalItems) + " mon hoc";
        gotoXY(x + (width - pageInfo1.length()) / 2, currentY + 1);
        cout << pageInfo1;
        gotoXY(x + (width - pageInfo2.length()) / 2, currentY + 2);
        cout << pageInfo2;
