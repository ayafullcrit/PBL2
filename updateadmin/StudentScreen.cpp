...
void ShowLichHoc_6(Student *student)
{
    ...
    total_height = 10 + 5 * student->GetSchedules().getSize();
    // Phân loại lịch học
    ...
    completedSchedules = SortSchedulesByTime(completedSchedules);
    cancelledSchedules = SortSchedulesByTime(cancelledSchedules);
    int currentY = y + 3;
...
void HandleStudentMenu(Student *student)
{
    ...
    {                               // 2. Bat dau vong lap su kien cho menu sinh vien.
        ...
        {
            FindTutorScreen(student); // Chay chuc nang tim kiem
